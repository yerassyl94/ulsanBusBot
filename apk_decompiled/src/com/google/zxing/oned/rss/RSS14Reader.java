...
public final class RSS14Reader extends AbstractRSSReader {
    ...
    public Result decodeRow(int rowNumber, BitArray row, Hashtable hints) throws NotFoundException {
        ...
        row.reverse();
        int numLeftPairs = this.possibleLeftPairs.size();
        int numRightPairs = this.possibleRightPairs.size();
        for (int l = 0; l < numLeftPairs; l++) {
            Pair left = (Pair) this.possibleLeftPairs.elementAt(l);
            if (left.getCount() > 1) {
                for (int r = 0; r < numRightPairs; r++) {
                    Pair right = (Pair) this.possibleRightPairs.elementAt(r);
