...
public final class ReedSolomonEncoder {
    ...
    public ReedSolomonEncoder(GenericGF field) {
        if (GenericGF.QR_CODE_FIELD_256.equals(field)) {
            ...
            this.cachedGenerators.addElement(new GenericGFPoly(field, new int[]{1}));
            return;
        }
    ...
    private GenericGFPoly buildGenerator(int degree) {
        if (degree >= this.cachedGenerators.size()) {
            GenericGFPoly lastGenerator = (GenericGFPoly) this.cachedGenerators.elementAt(this.cachedGenerators.size() - 1);
