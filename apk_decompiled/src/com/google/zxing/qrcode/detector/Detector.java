...
public class Detector {
    ...
    protected static int computeDimension(ResultPoint topLeft, ResultPoint topRight, ResultPoint bottomLeft, float moduleSize) throws NotFoundException {
        int dimension = ((round(ResultPoint.distance(topLeft, topRight) / moduleSize) + round(ResultPoint.distance(topLeft, bottomLeft) / moduleSize)) >> 1) + 7;
        switch (dimension & 3) {
            case 0:
                return dimension + 1;
            case 2:
    ...
    private float sizeOfBlackWhiteBlackRunBothWays(int fromX, int fromY, int toX, int toY) {
        ...
        return result + sizeOfBlackWhiteBlackRun(fromX, fromY, (int) (((float) fromX) + (((float) (otherToX - fromX)) * scale)), otherToY);
    }
